...
import type { AgentLeaderboardEntry } from "@/lib/api";
import { formatDuration } from "@/lib/utils";

interface AgentMiniLeaderboardProps {
  entries: AgentLeaderboardEntry[];
}

const RANK_COLORS = ["text-yellow-500", "text-zinc-400", "text-amber-600"];

export default function AgentMiniLeaderboard({ entries }: AgentMiniLeaderboardProps) {
  const top5 = entries.slice(0, 5);

  if (top5.length === 0) {
    return (
      <p className="py-4 text-center text-sm text-zinc-400">No agent data</p>
    );
  }
